...
public sealed class MediaItemViewModelTests
{
    ...
    [Theory]
    [InlineData(new[] { MediaStatus.Verified, MediaStatus.Verified }, "Verified", "#10B981")]
    [InlineData(new[] { MediaStatus.Verified, MediaStatus.Missing }, "Partial", "#F59E0B")]
    [InlineData(new[] { MediaStatus.Missing, MediaStatus.Missing }, "Missing", "#F43F5E")]
    [InlineData(new[] { MediaStatus.Corrupt, MediaStatus.Corrupt }, "Corrupt", "#F43F5E")]
    [InlineData(new[] { MediaStatus.Verified, MediaStatus.Corrupt }, "Corrupt", "#F43F5E")]
    [InlineData(new[] { MediaStatus.Corrupt, MediaStatus.Missing }, "Corrupt", "#F43F5E")]
    public void StatusText_FollowsEngineClassificationPrecedence(
        ...
        string expectedColor)
    {
        ...
        Assert.Equal(item.OverallStatus.ToString(), vm.StatusText);
